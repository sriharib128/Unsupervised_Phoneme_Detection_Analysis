...
def get_matched_gt(audio_file_path):
...
for audio_file in tqdm(os.listdir(audio_directory_path)):
    ...
    if audio_file.endswith('.wav'):  # or other formats, like '.mp3'
        matched_gt = get_matched_gt(audio_file_path)
        ans.append(matched_gt)

# Save the results as a JSON file
output_json_path = 'matched_results_test.json'  # Path to save the JSON file
...
# Save the results as a JSON file
output_json_path = 'matched_results_train.json'  # Path to save the JSON file
with open(output_json_path, 'w') as json_file:
    json.dump(ans, json_file, indent=0)
